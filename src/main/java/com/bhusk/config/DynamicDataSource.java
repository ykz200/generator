...
public class DynamicDataSource extends DataSource {
    ...
    private DataSource initDDS(String projectCode) throws IllegalArgumentException, IllegalAccessException {
        ...
        // 3、设置数据库名称和IP和端口号(一般来说用户名、密码都是统一固定的)
        String urlFormat = this.getUrl();
        String url = String.format(urlFormat, ProjectDBMgr.instance().getDBIP(projectCode), ProjectDBMgr.instance().getDBPR(projectCode), ProjectDBMgr.instance().getDBName(projectCode));
        dds.setUrl(url);
        String userFormat = ProjectDBMgr.dbUSMap.get(projectCode);
        String username = String.format(userFormat, ProjectDBMgr.instance().getDBUS(projectCode));
        dds.setUsername(username);
        String passwordFormat = ProjectDBMgr.dbPAMap.get(projectCode);
        String password = String.format(passwordFormat, ProjectDBMgr.instance().getDBPA(projectCode));
